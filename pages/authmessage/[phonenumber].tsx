            ...
            <div>
                ...
                <div className='m-3'>
                    ...
                    <div className='flex flex-col'>
                        <input type="text" placeholder={phoneValue} value={phoneValue} onChange={(evt) => { handleChange(evt, 1) }} className='px-3 text-xl my-4 bg-transparent border-solid border-2 border-[#C0C0C0] py-2 text-[#C0C0C0] rounded-lg' />
                        <button disabled={disabledRepeat} className={`rounded-lg border-solid border-2 py-3 text-2xl ${!disabledRepeat ? `border-[#C0C0C0] text-[#C0C0C0] font-bold` : `border-slate-600 text-slate-600`}`}>인증문자 다시 받기</button>
                        <input type="text" placeholder="인증번호를 입력해주세요" value={auth} onChange={(evt) => { handleChange(evt, 2) }} className="rounded-lg px-3 text-xl my-4 bg-transparent border-solid border-2 border-[#C0C0C0] py-2 text-[#C0C0C0]" />
                        <p className="font-normal text-lg text-[#C0C0C0]">어떤 경우에도 타인에게 공유하지 마세요!</p>
                        <button disabled={disabledAuth} onClick={handleAuthClick} className={`py-3 mt-2 rounded-lg py-2 text-2xl text-[#C0C0C0] font-bold ${!disabledAuth ? `bg-[#ff6f0f]` : ` bg-slate-600`}`}>인증번호 확인</button>
                    </div>
                </div>
            </div>
            <div hidden={authErrorHidden} className="p-2 bg-[#C0C0C0] text-lg">
